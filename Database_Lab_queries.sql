...
select ord.ORD_ID,ord.ORD_AMOUNT,ord.ORD_DATE,prod.PRO_NAME
 from `order` ord 
 inner join supplier_pricing supp 
on ord.pricing_id = supp.pricing_id 
inner join product prod 
on supp.pro_id = prod.pro_id
where ord.CUS_ID=2;

/*Question 6*/
select supp.* ,count(pro_id) as Total_products from supplier supp inner join supplier_pricing pricing
on supp.SUPP_ID=pricing.SUPP_ID
group by supp.SUPP_ID
having count(*) > 1;

/*Question 7*/

 select k.cat_id, k.cat_name,  product.pro_id, product.pro_name, k.minPrice 
